...
<body>
    ...
    <main class="container">
        <div class="search-page-content">
            ...
            if (isset($_GET['busca']) && !empty(trim($_GET['busca']))) {
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    while ($livro = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        if (!empty($livro['imagem'])) {
                            echo "<img src='../../uploads/" . htmlspecialchars($livro['imagem']) . "' class='capaLivros' alt='" . htmlspecialchars($livro['titulo']) . "'>";
                        } else {
                ...
                echo '<h3>Bem-vindo à nossa biblioteca!</h3>';
                echo '<p>Digite um termo na barra de pesquisa acima para encontrar livros ou autores.</p>';
                echo '</div>';
            }
            ?>
        </div>
    ...
    <script>
        ...
        // Validação da busca (opcional, pois o formulário já faz o submit)
        function validarBusca() {
            const termo = document.querySelector('input[name="busca"]').value.trim();
            if (termo === '') {
